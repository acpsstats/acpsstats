...
switch($type){
	case 'all' :
		$sql = "SELECT Count(tblmasterpoddata.ID) AS RecordCount, 
				Avg(If(AdmissionDate=SeparationDate,1,(SeparationDate-AdmissionDate))) AS ALOS
				FROM tblmasterpoddata
				WHERE (((tblmasterpoddata.Validated)=True) AND ((tblmasterpoddata.SurgeryDate) 
				Between '$fromdate_r' And '$todate_r')) {$query} {$query_condition}";
		break;
	case 'user'  :
		$sql = "SELECT tblmasterpoddata.UserName, tblmasterpoddata.UserState, Count(tblmasterpoddata.ID) AS RecordCount, 
				...
				GROUP BY tblmasterpoddata.UserName, tblmasterpoddata.UserState
...
<div class="row-fluid">
	...
	<div class="r-sub-heading">
		<?php if($type != "all"): ?>
			<h2>Average Length of Stay by Date Range</h2>
		<?php else: ?>
			<h3>Average Length of Stay by Date Range</h3>	
		<?php endif;?>
