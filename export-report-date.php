...
if(isset($_REQUEST['get_date_report'])){
	$field['show_record_bydate']='';
	$current_user = isset($_SESSION['user_id']) ? $_SESSION['user_id'] : '';
	$field['start_surgerydate']= isset($_REQUEST['start_surgerydate']) ? $_REQUEST['start_surgerydate'] : '';
	$field['end_surgerydate'] = isset($_REQUEST['end_surgerydate']) ? $_REQUEST['end_surgerydate'] : '';
	if($field['start_surgerydate']!='' && $field['end_surgerydate']!=''){
	 $field['show_record_bydate']=1;
}
$db_master_data=get_masterdata('','','',$field);
...
if(!empty($db_master_data)) {
		...
		$part = $part + 1;
		$filename = $filename.'_'.$field['end_surgerydate'].'.xls';
		...
		//$objWriter->save(str_replace(__FILE__,$_SERVER['DOCUMENT_ROOT'] .'/save/'.$filename,__FILE__));;
		exit;
...
?>
<style>
	.age-report-form input.error{border:1px solid #F00;}
	.error_msg {padding: 15px 20px;background-color: #FFF;width: 60%;margin: 30px auto;text-align:center;}
</style>
<div class="row-fluid age-report-form" style='float:left;padding:15px;'>
 <form name="age-report-form" method="post" >
                <div class="span4"> 
                	    <label>Start SurgeryDate: </label>
                        <input type="text" name="start_surgerydate"  value="01/01/2015" class="datepicker span11 req" style='float:left;'/>
                </div>
                 <div class="span4"> 
                 	     <label>End SurgeryDate: </label>
                         <input type="text" name="end_surgerydate"  value="01/12/2015" class="datepicker span11 req" style='float:left;'/>
                </div>
                <div class="span4">
                         <label style='color:transparent;'> Report by Excel:</label>  
                         <input type="submit" name="get_date_report" class="btn btn-info get_date_report span6" value="Submit" style='float:left;' /> 
		<?php if(isset($_REQUEST["get_report"]) || isset($_REQUEST["ft"]) ) {   ?>  
                                 <input type="submit" name="ft"  value="Excel" class="btn btn-info  get_date_report span4" style='float:left;margin-left:10px;'/> 
        <?php } ?>
</div>
</form>
<?php if(empty( $db_master_data) && isset($field['start_surgerydate'])) {
  $error_msg='There is no records between '.$field['start_surgerydate'].' and '.$field['end_surgerydate'];
?>
<div class="span8 error_msg">
  <?php  echo  $error_msg; ?>
</div>
<?php } ?> 
</div>  
