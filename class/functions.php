...
$uri = trim(eregi_replace('http', '', $uri));
$url = 'http://siteexplorer.search.yahoo.com/advsearch?p=http://'.
$uri.'&bwm=i&bwmf=s&bwmo=&fr2=seo-rd-se';
$v = file_get_contents_curl($url);
preg_match('/of about \<strong\>(.*?) \<\/strong\>/si',$v,$r);
return ($r[1]) ? $r[1] : '0';
}*/
...
function yahoo_indexed($uri)
{
   ...
   // $url="muthuonline.com";
   //// $url_content = 'http://siteexplorer.search.yahoo.com/advsearch?p=http://muthuonline.com&bwm=p&bwmf=s&bwmo=d';
	$aa=file_get_contents($url);
	$ab=strpos($aa,'<span class="btn">Pages ('); 
	$final=substr($aa,$ab,40);
	$value=str_replace("(","",$final);
...
function msn_indexed($uri)
{
    ...
    $data = file_get_contents_curl($url);
    $a=strpos($data,'<span class="sb_count" id="count">');
	$ab=substr($data,$a,100);
	$aaa=substr($ab,strpos($ab,"of "),strpos($ab,"results"));
	$result=str_replace("of","",$aaa);
	$result=str_replace("results","",$result);
	$result=str_replace(",","",$result);
	$split=explode(" ",$result);
	$final=trim($split[1]);	
	$value=strip_tags($final);
...
function getLanguage($text){
	...
	$google_api="http://www.translate.google.com/translate_t?hl=en&sl=en&tl=".$_SESSION['lang_pack']."&q=".$query."";
	$content=file_get_contents($google_api);
	$first='dir="ltr">';
	$ss=strpos($content,$first);
	$sec=substr($content,$ss+10);
	$second='</div>';
	$sss=strpos($sec,$second);
	$val=substr($sec,0,$sss);	
	return $val;
